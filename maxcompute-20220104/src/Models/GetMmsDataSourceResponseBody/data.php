...
class data extends Model
{
    ...
    public function validate()
    {
        ...
        if (\is_array($this->dstProjects)) {
            Model::validateArray($this->dstProjects);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->dstProject) {
            $res['dstProject'] = $this->dstProject;
        }

        if (null !== $this->dstProjects) {
            if (\is_array($this->dstProjects)) {
                $res['dstProjects'] = [];
                $n1 = 0;
                foreach ($this->dstProjects as $item1) {
                    $res['dstProjects'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['dstProject'])) {
            $model->dstProject = $map['dstProject'];
        }

        if (isset($map['dstProjects'])) {
            if (!empty($map['dstProjects'])) {
                $model->dstProjects = [];
                $n1 = 0;
                foreach ($map['dstProjects'] as $item1) {
                    $model->dstProjects[$n1] = $item1;
                    ++$n1;
                }
            }
        }
