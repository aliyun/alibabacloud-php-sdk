...
namespace AlibabaCloud\SDK\Dysms\V20170620\Models\QueryMonthlyBillInfoLeafNewResponseBody;

use AlibabaCloud\Dara\Model;
use AlibabaCloud\SDK\Dysms\V20170620\Models\QueryMonthlyBillInfoLeafNewResponseBody\layeredBillDOList\subjectDetailList;

class layeredBillDOList extends Model
{
    /**
     * @var int
     */
    ...
    {
        if (\is_array($this->subjectDetailList)) {
            Model::validateArray($this->subjectDetailList);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->subjectDetailList) {
            if (\is_array($this->subjectDetailList)) {
                $res['SubjectDetailList'] = [];
                $n1 = 0;
                foreach ($this->subjectDetailList as $item1) {
                    $res['SubjectDetailList'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['SubjectDetailList'])) {
            if (!empty($map['SubjectDetailList'])) {
                $model->subjectDetailList = [];
                $n1 = 0;
                foreach ($map['SubjectDetailList'] as $item1) {
                    $model->subjectDetailList[$n1] = subjectDetailList::fromMap($item1);
                    ++$n1;
                }
            }
        }
