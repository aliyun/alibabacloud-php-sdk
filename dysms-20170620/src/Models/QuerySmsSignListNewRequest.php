...
class QuerySmsSignListNewRequest extends Model
{
    ...
    {
        if (\is_array($this->operatorCodes)) {
            Model::validateArray($this->operatorCodes);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->operatorCodes) {
            if (\is_array($this->operatorCodes)) {
                $res['OperatorCodes'] = [];
                $n1 = 0;
                foreach ($this->operatorCodes as $item1) {
                    $res['OperatorCodes'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['OperatorCodes'])) {
            if (!empty($map['OperatorCodes'])) {
                $model->operatorCodes = [];
                $n1 = 0;
                foreach ($map['OperatorCodes'] as $item1) {
                    $model->operatorCodes[$n1] = $item1;
                    ++$n1;
                }
            }
        }
