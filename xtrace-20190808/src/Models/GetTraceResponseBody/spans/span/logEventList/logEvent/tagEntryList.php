<?php

// This file is auto-generated, don't edit it. Thanks.

namespace AlibabaCloud\SDK\Xtrace\V20190808\Models\GetTraceResponseBody\spans\span\logEventList\logEvent;

use AlibabaCloud\Dara\Model;
use AlibabaCloud\SDK\Xtrace\V20190808\Models\GetTraceResponseBody\spans\span\logEventList\logEvent\tagEntryList\tagEntry;

class tagEntryList extends Model
{
    /**
     * @var tagEntry[]
     */
    public $tagEntry;
    protected $_name = [
        'tagEntry' => 'TagEntry',
    ];

    public function validate()
    {
        if (\is_array($this->tagEntry)) {
            Model::validateArray($this->tagEntry);
        }
        parent::validate();
    }

    public function toArray($noStream = false)
    {
        $res = [];
        if (null !== $this->tagEntry) {
            if (\is_array($this->tagEntry)) {
                $res['TagEntry'] = [];
                $n1 = 0;
                foreach ($this->tagEntry as $item1) {
                    $res['TagEntry'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }

        return $res;
    }

    public function toMap($noStream = false)
    {
        return $this->toArray($noStream);
    }

    public static function fromMap($map = [])
    {
        $model = new self();
        if (isset($map['TagEntry'])) {
            if (!empty($map['TagEntry'])) {
                $model->tagEntry = [];
                $n1 = 0;
                foreach ($map['TagEntry'] as $item1) {
                    $model->tagEntry[$n1] = tagEntry::fromMap($item1);
                    ++$n1;
                }
            }
        }

        return $model;
    }
}
