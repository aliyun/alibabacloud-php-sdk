...
class CreateDedicatedHostRequest extends Model
{
    ...
    {
        if (\is_array($this->clusterServices)) {
            Model::validateArray($this->clusterServices);
        }
        if (null !== $this->myBaseEcsClass) {
            $this->myBaseEcsClass->validate();
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->clusterServices) {
            if (\is_array($this->clusterServices)) {
                $res['ClusterServices'] = [];
                $n1 = 0;
                foreach ($this->clusterServices as $item1) {
                    $res['ClusterServices'][$n1++] = $item1;
                }
            }
        }
        ...
        if (null !== $this->myBaseEcsClass) {
            $res['MyBaseEcsClass'] = null !== $this->myBaseEcsClass ? $this->myBaseEcsClass->toArray($noStream) : $this->myBaseEcsClass;
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ClusterServices'])) {
            if (!empty($map['ClusterServices'])) {
                $model->clusterServices = [];
                $n1 = 0;
                foreach ($map['ClusterServices'] as $item1) {
                    $model->clusterServices[$n1++] = $item1;
                }
            }
        }
