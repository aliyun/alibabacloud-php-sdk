...
use AlibabaCloud\SDK\Vpc\V20160428\Models\ListBusinessRegionsResponseBody\geographicSubRegions;

class ListBusinessRegionsResponseBody extends Model
{
    /**
     * @var int
     */
        ...
        if (null !== $this->geographicSubRegions) {
            if (\is_array($this->geographicSubRegions)) {
                ...
                foreach ($this->geographicSubRegions as $item1) {
                    $res['GeographicSubRegions'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
        ...
        if (isset($map['GeographicSubRegions'])) {
            if (!empty($map['GeographicSubRegions'])) {
                ...
                foreach ($map['GeographicSubRegions'] as $item1) {
                    $model->geographicSubRegions[$n1] = geographicSubRegions::fromMap($item1);
                    ++$n1;
                }
            }
        }
