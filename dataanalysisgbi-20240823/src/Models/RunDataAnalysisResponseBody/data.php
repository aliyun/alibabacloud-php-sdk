...
use AlibabaCloud\SDK\DataAnalysisGBI\V20240823\Models\RunDataAnalysisResponseBody\data\chat;
...
class data extends Model
{
    ...
    public function validate()
    {
        if (\is_array($this->attempts)) {
            Model::validateArray($this->attempts);
        }
        if (null !== $this->chat) {
            $this->chat->validate();
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->attempts) {
            if (\is_array($this->attempts)) {
                $res['attempts'] = [];
                $n1 = 0;
                foreach ($this->attempts as $item1) {
                    $res['attempts'][$n1++] = $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['attempts'])) {
            if (!empty($map['attempts'])) {
                $model->attempts = [];
                $n1 = 0;
                foreach ($map['attempts'] as $item1) {
                    $model->attempts[$n1++] = $item1;
                }
            }
        }
