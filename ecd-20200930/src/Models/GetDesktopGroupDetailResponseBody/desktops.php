...
class desktops extends Model
{
    ...
    {
        if (\is_array($this->expiredTimes)) {
            Model::validateArray($this->expiredTimes);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->expiredTimes) {
            if (\is_array($this->expiredTimes)) {
                $res['ExpiredTimes'] = [];
                $n1 = 0;
                foreach ($this->expiredTimes as $item1) {
                    $res['ExpiredTimes'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ExpiredTimes'])) {
            if (!empty($map['ExpiredTimes'])) {
                $model->expiredTimes = [];
                $n1 = 0;
                foreach ($map['ExpiredTimes'] as $item1) {
                    $model->expiredTimes[$n1] = $item1;
                    ++$n1;
                }
            }
        }
