...
class openJMeterScene extends Model
{
    ...
    public function validate()
    {
        ...
        if (\is_array($this->JMeterProperties)) {
            Model::validateArray($this->JMeterProperties);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->JMeterProperties) {
            if (\is_array($this->JMeterProperties)) {
                ...
                $n1 = 0;
                foreach ($this->JMeterProperties as $item1) {
                    $res['JMeterProperties'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['JMeterProperties'])) {
            if (!empty($map['JMeterProperties'])) {
                ...
                $n1 = 0;
                foreach ($map['JMeterProperties'] as $item1) {
                    $model->JMeterProperties[$n1] = JMeterProperties::fromMap($item1);
                    ++$n1;
                }
            }
        }
