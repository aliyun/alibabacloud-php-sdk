...
class DescribeLiveDomainPublishErrorCodeResponseBody extends Model
{
    ...
    {
        if (\is_array($this->realTimeCodeData)) {
            Model::validateArray($this->realTimeCodeData);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->realTimeCodeData) {
            if (\is_array($this->realTimeCodeData)) {
                ...
                $n1 = 0;
                foreach ($this->realTimeCodeData as $item1) {
                    $res['RealTimeCodeData'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['RealTimeCodeData'])) {
            if (!empty($map['RealTimeCodeData'])) {
                ...
                $n1 = 0;
                foreach ($map['RealTimeCodeData'] as $item1) {
                    $model->realTimeCodeData[$n1] = realTimeCodeData::fromMap($item1);
                    ++$n1;
                }
            }
        }
