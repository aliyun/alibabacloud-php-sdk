...
class CreateOrderRequest extends Model
{
    ...
    {
        if (\is_array($this->pluginParam)) {
            Model::validateArray($this->pluginParam);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->pluginParam) {
            if (\is_array($this->pluginParam)) {
                $res['PluginParam'] = [];
                foreach ($this->pluginParam as $key1 => $value1) {
                    $res['PluginParam'][$key1] = $value1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['PluginParam'])) {
            if (!empty($map['PluginParam'])) {
                $model->pluginParam = [];
                foreach ($map['PluginParam'] as $key1 => $value1) {
                    $model->pluginParam[$key1] = $value1;
                }
            }
        }
