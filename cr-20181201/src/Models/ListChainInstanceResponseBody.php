...
class ListChainInstanceResponseBody extends Model
{
    ...
    {
        if (\is_array($this->chainInstances)) {
            Model::validateArray($this->chainInstances);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->chainInstances) {
            if (\is_array($this->chainInstances)) {
                $res['ChainInstances'] = [];
                $n1 = 0;
                foreach ($this->chainInstances as $item1) {
                    $res['ChainInstances'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ChainInstances'])) {
            if (!empty($map['ChainInstances'])) {
                ...
                $n1 = 0;
                foreach ($map['ChainInstances'] as $item1) {
                    $model->chainInstances[$n1] = chainInstances::fromMap($item1);
                    ++$n1;
                }
            }
        }
