...
class ListResourceTypesRequest extends Model
{
    ...
    {
        if (\is_array($this->terraformResourceTypes)) {
            Model::validateArray($this->terraformResourceTypes);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->terraformResourceTypes) {
            if (\is_array($this->terraformResourceTypes)) {
                $res['terraformResourceTypes'] = [];
                $n1 = 0;
                foreach ($this->terraformResourceTypes as $item1) {
                    $res['terraformResourceTypes'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['terraformResourceTypes'])) {
            if (!empty($map['terraformResourceTypes'])) {
                $model->terraformResourceTypes = [];
                $n1 = 0;
                foreach ($map['terraformResourceTypes'] as $item1) {
                    $model->terraformResourceTypes[$n1] = $item1;
                    ++$n1;
                }
            }
        }
