...
class CreateTenantRequest extends Model
{
    ...
    {
        if (\is_array($this->createParams)) {
            Model::validateArray($this->createParams);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->createParams) {
            if (\is_array($this->createParams)) {
                $res['CreateParams'] = [];
                foreach ($this->createParams as $key1 => $value1) {
                    $res['CreateParams'][$key1] = $value1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['CreateParams'])) {
            if (!empty($map['CreateParams'])) {
                $model->createParams = [];
                foreach ($map['CreateParams'] as $key1 => $value1) {
                    $model->createParams[$key1] = $value1;
                }
            }
        }
