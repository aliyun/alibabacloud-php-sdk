...
class ListIncidentsRequest extends Model
{
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->incidentUuids) {
            if (\is_array($this->incidentUuids)) {
                $res['IncidentUuids'] = [];
                $n1 = 0;
                foreach ($this->incidentUuids as $item1) {
                    $res['IncidentUuids'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['IncidentUuids'])) {
            if (!empty($map['IncidentUuids'])) {
                $model->incidentUuids = [];
                $n1 = 0;
                foreach ($map['IncidentUuids'] as $item1) {
                    $model->incidentUuids[$n1] = $item1;
                    ++$n1;
                }
            }
        }
