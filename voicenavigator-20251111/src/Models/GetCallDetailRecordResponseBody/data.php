...
namespace AlibabaCloud\SDK\VoiceNavigator\V20251111\Models\GetCallDetailRecordResponseBody;

use AlibabaCloud\Dara\Model;
use AlibabaCloud\SDK\VoiceNavigator\V20251111\Models\GetCallDetailRecordResponseBody\data\transcripts;
...
class data extends Model
{
    ...
    {
        if (\is_array($this->transcripts)) {
            Model::validateArray($this->transcripts);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->transcripts) {
            if (\is_array($this->transcripts)) {
                $res['Transcripts'] = [];
                $n1 = 0;
                foreach ($this->transcripts as $item1) {
                    $res['Transcripts'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['Transcripts'])) {
            if (!empty($map['Transcripts'])) {
                $model->transcripts = [];
                $n1 = 0;
                foreach ($map['Transcripts'] as $item1) {
                    $model->transcripts[$n1] = transcripts::fromMap($item1);
                    ++$n1;
                }
            }
        }
