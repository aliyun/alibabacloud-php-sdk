...
class ListServiceUsagesResponseBody extends Model
{
    ...
    {
        if (\is_array($this->serviceUsages)) {
            Model::validateArray($this->serviceUsages);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->serviceUsages) {
            if (\is_array($this->serviceUsages)) {
                $res['ServiceUsages'] = [];
                $n1 = 0;
                foreach ($this->serviceUsages as $item1) {
                    $res['ServiceUsages'][$n1++] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ServiceUsages'])) {
            if (!empty($map['ServiceUsages'])) {
                $model->serviceUsages = [];
                $n1 = 0;
                foreach ($map['ServiceUsages'] as $item1) {
                    $model->serviceUsages[$n1++] = serviceUsages::fromMap($item1);
                }
            }
        }
