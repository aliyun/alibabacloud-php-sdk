...
class Elasticsearch extends OpenApiClient
{
     ...
     */
    public function listAllNode($InstanceId)
    {
        $runtime = new RuntimeOptions([]);
        $headers = [];

        return $this->listAllNodeWithOptions($InstanceId, $headers, $runtime);
    }
     ...
     * @return ListAllNodeResponse
     */
    public function listAllNodeWithOptions($InstanceId, $headers, $runtime)
    {
        $InstanceId = OpenApiUtilClient::getEncodeParam($InstanceId);
        $req        = new OpenApiRequest([
            'headers' => $headers,
