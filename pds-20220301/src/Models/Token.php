...
class Token extends Model
{
    ...
    {
        if (\is_array($this->existLink)) {
            Model::validateArray($this->existLink);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->existLink) {
            if (\is_array($this->existLink)) {
                $res['exist_link'] = [];
                $n1 = 0;
                foreach ($this->existLink as $item1) {
                    $res['exist_link'][$n1++] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['exist_link'])) {
            if (!empty($map['exist_link'])) {
                $model->existLink = [];
                $n1 = 0;
                foreach ($map['exist_link'] as $item1) {
                    $model->existLink[$n1++] = LinkInfo::fromMap($item1);
                }
            }
        }
