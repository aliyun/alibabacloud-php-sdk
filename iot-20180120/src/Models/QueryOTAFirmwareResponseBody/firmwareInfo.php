...
class firmwareInfo extends Model
{
    ...
    public function validate()
    {
        if (\is_array($this->multiFiles)) {
            Model::validateArray($this->multiFiles);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->multiFiles) {
            if (\is_array($this->multiFiles)) {
                $res['MultiFiles'] = [];
                $n1 = 0;
                foreach ($this->multiFiles as $item1) {
                    $res['MultiFiles'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['MultiFiles'])) {
            if (!empty($map['MultiFiles'])) {
                $model->multiFiles = [];
                $n1 = 0;
                foreach ($map['MultiFiles'] as $item1) {
                    $model->multiFiles[$n1] = multiFiles::fromMap($item1);
                    ++$n1;
                }
            }
        }
