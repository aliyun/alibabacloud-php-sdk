<?php

// This file is auto-generated, don't edit it. Thanks.

namespace AlibabaCloud\SDK\Notifications\V20241225\Models;

use AlibabaCloud\Dara\Model;

class UpdateUserSubscriptionShrinkRequest extends Model
{
    /**
     * @var string
     */
    public $acceptLanguage;

    /**
     * @var string
     */
    public $appName;

    /**
     * @var string
     */
    public $bizName;

    /**
     * @var string
     */
    public $callerProtocol;

    /**
     * @var string
     */
    public $categoryCodesShrink;

    /**
     * @var string
     */
    public $channelConfigsShrink;

    /**
     * @var string
     */
    public $channelGroupCode;

    /**
     * @var string
     */
    public $clientSource;

    /**
     * @var string
     */
    public $commonContactsShrink;

    /**
     * @var string
     */
    public $cookies;

    /**
     * @var string
     */
    public $operationItemCode;

    /**
     * @var string
     */
    public $receiveTimeListShrink;

    /**
     * @var string
     */
    public $remarks;

    /**
     * @var string
     */
    public $srcUrl;

    /**
     * @var string
     */
    public $tenantCode;

    /**
     * @var string
     */
    public $uidType;

    /**
     * @var string
     */
    public $webhookContactsShrink;
    protected $_name = [
        'acceptLanguage' => 'AcceptLanguage',
        'appName' => 'AppName',
        'bizName' => 'BizName',
        'callerProtocol' => 'CallerProtocol',
        'categoryCodesShrink' => 'CategoryCodes',
        'channelConfigsShrink' => 'ChannelConfigs',
        'channelGroupCode' => 'ChannelGroupCode',
        'clientSource' => 'ClientSource',
        'commonContactsShrink' => 'CommonContacts',
        'cookies' => 'Cookies',
        'operationItemCode' => 'OperationItemCode',
        'receiveTimeListShrink' => 'ReceiveTimeList',
        'remarks' => 'Remarks',
        'srcUrl' => 'SrcUrl',
        'tenantCode' => 'TenantCode',
        'uidType' => 'UidType',
        'webhookContactsShrink' => 'WebhookContacts',
    ];

    public function validate()
    {
        parent::validate();
    }

    public function toArray($noStream = false)
    {
        $res = [];
        if (null !== $this->acceptLanguage) {
            $res['AcceptLanguage'] = $this->acceptLanguage;
        }

        if (null !== $this->appName) {
            $res['AppName'] = $this->appName;
        }

        if (null !== $this->bizName) {
            $res['BizName'] = $this->bizName;
        }

        if (null !== $this->callerProtocol) {
            $res['CallerProtocol'] = $this->callerProtocol;
        }

        if (null !== $this->categoryCodesShrink) {
            $res['CategoryCodes'] = $this->categoryCodesShrink;
        }

        if (null !== $this->channelConfigsShrink) {
            $res['ChannelConfigs'] = $this->channelConfigsShrink;
        }

        if (null !== $this->channelGroupCode) {
            $res['ChannelGroupCode'] = $this->channelGroupCode;
        }

        if (null !== $this->clientSource) {
            $res['ClientSource'] = $this->clientSource;
        }

        if (null !== $this->commonContactsShrink) {
            $res['CommonContacts'] = $this->commonContactsShrink;
        }

        if (null !== $this->cookies) {
            $res['Cookies'] = $this->cookies;
        }

        if (null !== $this->operationItemCode) {
            $res['OperationItemCode'] = $this->operationItemCode;
        }

        if (null !== $this->receiveTimeListShrink) {
            $res['ReceiveTimeList'] = $this->receiveTimeListShrink;
        }

        if (null !== $this->remarks) {
            $res['Remarks'] = $this->remarks;
        }

        if (null !== $this->srcUrl) {
            $res['SrcUrl'] = $this->srcUrl;
        }

        if (null !== $this->tenantCode) {
            $res['TenantCode'] = $this->tenantCode;
        }

        if (null !== $this->uidType) {
            $res['UidType'] = $this->uidType;
        }

        if (null !== $this->webhookContactsShrink) {
            $res['WebhookContacts'] = $this->webhookContactsShrink;
        }

        return $res;
    }

    public function toMap($noStream = false)
    {
        return $this->toArray($noStream);
    }

    public static function fromMap($map = [])
    {
        $model = new self();
        if (isset($map['AcceptLanguage'])) {
            $model->acceptLanguage = $map['AcceptLanguage'];
        }

        if (isset($map['AppName'])) {
            $model->appName = $map['AppName'];
        }

        if (isset($map['BizName'])) {
            $model->bizName = $map['BizName'];
        }

        if (isset($map['CallerProtocol'])) {
            $model->callerProtocol = $map['CallerProtocol'];
        }

        if (isset($map['CategoryCodes'])) {
            $model->categoryCodesShrink = $map['CategoryCodes'];
        }

        if (isset($map['ChannelConfigs'])) {
            $model->channelConfigsShrink = $map['ChannelConfigs'];
        }

        if (isset($map['ChannelGroupCode'])) {
            $model->channelGroupCode = $map['ChannelGroupCode'];
        }

        if (isset($map['ClientSource'])) {
            $model->clientSource = $map['ClientSource'];
        }

        if (isset($map['CommonContacts'])) {
            $model->commonContactsShrink = $map['CommonContacts'];
        }

        if (isset($map['Cookies'])) {
            $model->cookies = $map['Cookies'];
        }

        if (isset($map['OperationItemCode'])) {
            $model->operationItemCode = $map['OperationItemCode'];
        }

        if (isset($map['ReceiveTimeList'])) {
            $model->receiveTimeListShrink = $map['ReceiveTimeList'];
        }

        if (isset($map['Remarks'])) {
            $model->remarks = $map['Remarks'];
        }

        if (isset($map['SrcUrl'])) {
            $model->srcUrl = $map['SrcUrl'];
        }

        if (isset($map['TenantCode'])) {
            $model->tenantCode = $map['TenantCode'];
        }

        if (isset($map['UidType'])) {
            $model->uidType = $map['UidType'];
        }

        if (isset($map['WebhookContacts'])) {
            $model->webhookContactsShrink = $map['WebhookContacts'];
        }

        return $model;
    }
}
