...
class ListQuotaApplicationsDetailForTemplateResponseBody extends Model
{
    ...
    {
        if (\is_array($this->quotaApplications)) {
            Model::validateArray($this->quotaApplications);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->quotaApplications) {
            if (\is_array($this->quotaApplications)) {
                ...
                $n1 = 0;
                foreach ($this->quotaApplications as $item1) {
                    $res['QuotaApplications'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['QuotaApplications'])) {
            if (!empty($map['QuotaApplications'])) {
                ...
                $n1 = 0;
                foreach ($map['QuotaApplications'] as $item1) {
                    $model->quotaApplications[$n1] = quotaApplications::fromMap($item1);
                    ++$n1;
                }
            }
        }
