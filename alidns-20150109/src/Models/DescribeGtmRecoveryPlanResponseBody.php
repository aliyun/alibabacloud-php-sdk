...
class DescribeGtmRecoveryPlanResponseBody extends Model
{
    ...
    public function toMap()
    {
        ...
        if (null !== $this->faultAddrPools) {
            $res['FaultAddrPools'] = [];
            if (null !== $this->faultAddrPools && \is_array($this->faultAddrPools)) {
                $n = 0;
                foreach ($this->faultAddrPools as $item) {
                    $res['FaultAddrPools'][$n++] = null !== $item ? $item->toMap() : $item;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['FaultAddrPools'])) {
            if (!empty($map['FaultAddrPools'])) {
                $model->faultAddrPools = [];
                $n                     = 0;
                foreach ($map['FaultAddrPools'] as $item) {
                    $model->faultAddrPools[$n++] = null !== $item ? faultAddrPools::fromMap($item) : $item;
                }
            }
        }
