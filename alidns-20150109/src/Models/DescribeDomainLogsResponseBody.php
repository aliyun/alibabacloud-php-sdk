...
class DescribeDomainLogsResponseBody extends Model
{
    ...
    public function toMap()
    {
        ...
        if (null !== $this->domainLogs) {
            $res['DomainLogs'] = [];
            if (null !== $this->domainLogs && \is_array($this->domainLogs)) {
                $n = 0;
                foreach ($this->domainLogs as $item) {
                    $res['DomainLogs'][$n++] = null !== $item ? $item->toMap() : $item;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['DomainLogs'])) {
            if (!empty($map['DomainLogs'])) {
                $model->domainLogs = [];
                $n                 = 0;
                foreach ($map['DomainLogs'] as $item) {
                    $model->domainLogs[$n++] = null !== $item ? domainLogs::fromMap($item) : $item;
                }
            }
        }
