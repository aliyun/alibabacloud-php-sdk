...
class DescribeTransferDomainsResponseBody extends Model
{
    ...
    public function toMap()
    {
        ...
        if (null !== $this->domainTransfers) {
            $res['DomainTransfers'] = [];
            if (null !== $this->domainTransfers && \is_array($this->domainTransfers)) {
                $n = 0;
                foreach ($this->domainTransfers as $item) {
                    $res['DomainTransfers'][$n++] = null !== $item ? $item->toMap() : $item;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['DomainTransfers'])) {
            if (!empty($map['DomainTransfers'])) {
                $model->domainTransfers = [];
                $n                      = 0;
                foreach ($map['DomainTransfers'] as $item) {
                    $model->domainTransfers[$n++] = null !== $item ? domainTransfers::fromMap($item) : $item;
                }
            }
        }
