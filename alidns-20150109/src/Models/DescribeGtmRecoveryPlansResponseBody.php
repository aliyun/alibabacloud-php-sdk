...
class DescribeGtmRecoveryPlansResponseBody extends Model
{
    ...
    public function toMap()
    {
        ...
        if (null !== $this->recoveryPlans) {
            $res['RecoveryPlans'] = [];
            if (null !== $this->recoveryPlans && \is_array($this->recoveryPlans)) {
                $n = 0;
                foreach ($this->recoveryPlans as $item) {
                    $res['RecoveryPlans'][$n++] = null !== $item ? $item->toMap() : $item;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['RecoveryPlans'])) {
            if (!empty($map['RecoveryPlans'])) {
                $model->recoveryPlans = [];
                $n                    = 0;
                foreach ($map['RecoveryPlans'] as $item) {
                    $model->recoveryPlans[$n++] = null !== $item ? recoveryPlans::fromMap($item) : $item;
                }
            }
        }
