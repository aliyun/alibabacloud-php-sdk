...
class ModifyInvocationAttributeRequest extends Model
{
    /**
    ...
    /**
     * @description The frequency at which to run the command. This parameter takes effect only when `RepeatMode` is set to `Period`. You can configure a command to run at a fixed interval based on a rate expression, run only once at a specified point in time, or run at designated points in time based on a cron expression.
     *
    ...
    /**
     * @description The IDs of the Elastic Compute Service (ECS) instances or managed instances to be added to the execution information of the command that is run on schedule. The total number of instances after you add ECS instances or managed instances to the execution information of the command cannot exceed 100.
     *
     * @var string[]
     */
    public $instanceId;
    ...
    /**
     * @description The key-value pairs of the custom parameters that were passed in if custom parameters are included in the command.
     *
     ...
     * @param array $map
     *
     * @return ModifyInvocationAttributeRequest
     */
