...
class ListFeatureViewsRequest extends Model
{
    ...
    {
        if (\is_array($this->featureViewIds)) {
            Model::validateArray($this->featureViewIds);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->featureViewIds) {
            if (\is_array($this->featureViewIds)) {
                $res['FeatureViewIds'] = [];
                $n1 = 0;
                foreach ($this->featureViewIds as $item1) {
                    $res['FeatureViewIds'][$n1++] = $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['FeatureViewIds'])) {
            if (!empty($map['FeatureViewIds'])) {
                $model->featureViewIds = [];
                $n1 = 0;
                foreach ($map['FeatureViewIds'] as $item1) {
                    $model->featureViewIds[$n1++] = $item1;
                }
            }
        }
