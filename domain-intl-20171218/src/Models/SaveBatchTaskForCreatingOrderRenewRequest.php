...
use AlibabaCloud\SDK\Domainintl\V20171218\Models\SaveBatchTaskForCreatingOrderRenewRequest\orderRenewParam;
...
class SaveBatchTaskForCreatingOrderRenewRequest extends Model
{
    ...
    {
        if (\is_array($this->orderRenewParam)) {
            Model::validateArray($this->orderRenewParam);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->orderRenewParam) {
            if (\is_array($this->orderRenewParam)) {
                $res['OrderRenewParam'] = [];
                $n1 = 0;
                foreach ($this->orderRenewParam as $item1) {
                    $res['OrderRenewParam'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['OrderRenewParam'])) {
            if (!empty($map['OrderRenewParam'])) {
                ...
                $n1 = 0;
                foreach ($map['OrderRenewParam'] as $item1) {
                    $model->orderRenewParam[$n1] = orderRenewParam::fromMap($item1);
                    ++$n1;
                }
            }
        }
