...
class data extends Model
{
    ...
    {
        if (\is_array($this->additionType)) {
            Model::validateArray($this->additionType);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->additionType) {
            if (\is_array($this->additionType)) {
                $res['AdditionType'] = [];
                $n1 = 0;
                foreach ($this->additionType as $item1) {
                    $res['AdditionType'][$n1] = $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['AdditionType'])) {
            if (!empty($map['AdditionType'])) {
                $model->additionType = [];
                $n1 = 0;
                foreach ($map['AdditionType'] as $item1) {
                    $model->additionType[$n1] = $item1;
                    ++$n1;
                }
            }
        }
