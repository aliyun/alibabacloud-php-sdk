...
class repairConfigs extends Model
{
    ...
    {
        if (\is_array($this->exclusiveName)) {
            Model::validateArray($this->exclusiveName);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->exclusiveName) {
            if (\is_array($this->exclusiveName)) {
                $res['ExclusiveName'] = [];
                $n1                   = 0;
                foreach ($this->exclusiveName as $item1) {
                    $res['ExclusiveName'][$n1++] = $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ExclusiveName'])) {
            if (!empty($map['ExclusiveName'])) {
                $model->exclusiveName = [];
                $n1                   = 0;
                foreach ($map['ExclusiveName'] as $item1) {
                    $model->exclusiveName[$n1++] = $item1;
                }
            }
        }
