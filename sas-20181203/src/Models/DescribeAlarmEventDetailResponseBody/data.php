...
class data extends Model
{
    ...
    {
        if (\is_array($this->causeDetails)) {
            Model::validateArray($this->causeDetails);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->causeDetails) {
            if (\is_array($this->causeDetails)) {
                $res['CauseDetails'] = [];
                $n1 = 0;
                foreach ($this->causeDetails as $item1) {
                    $res['CauseDetails'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['CauseDetails'])) {
            if (!empty($map['CauseDetails'])) {
                $model->causeDetails = [];
                $n1 = 0;
                foreach ($map['CauseDetails'] as $item1) {
                    $model->causeDetails[$n1] = causeDetails::fromMap($item1);
                    ++$n1;
                }
            }
        }
