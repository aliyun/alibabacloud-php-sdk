...
class GetDataSourceDependenciesResponseBody extends Model
{
    ...
    {
        if (\is_array($this->dependencyList)) {
            Model::validateArray($this->dependencyList);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->dependencyList) {
            if (\is_array($this->dependencyList)) {
                $res['DependencyList'] = [];
                $n1 = 0;
                foreach ($this->dependencyList as $item1) {
                    $res['DependencyList'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['DependencyList'])) {
            if (!empty($map['DependencyList'])) {
                ...
                $n1 = 0;
                foreach ($map['DependencyList'] as $item1) {
                    $model->dependencyList[$n1] = dependencyList::fromMap($item1);
                    ++$n1;
                }
            }
        }
