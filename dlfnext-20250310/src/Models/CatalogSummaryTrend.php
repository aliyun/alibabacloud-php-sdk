...
class CatalogSummaryTrend extends Model
{
    ...
    public function validate()
    {
        ...
        if (\is_array($this->fileAccessCount)) {
            Model::validateArray($this->fileAccessCount);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->fileAccessCount) {
            if (\is_array($this->fileAccessCount)) {
                $res['fileAccessCount'] = [];
                $n1 = 0;
                foreach ($this->fileAccessCount as $item1) {
                    $res['fileAccessCount'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['fileAccessCount'])) {
            if (!empty($map['fileAccessCount'])) {
                ...
                $n1 = 0;
                foreach ($map['fileAccessCount'] as $item1) {
                    $model->fileAccessCount[$n1] = DateSummary::fromMap($item1);
                    ++$n1;
                }
            }
        }
