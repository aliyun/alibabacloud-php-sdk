...
namespace AlibabaCloud\SDK\Appflow\V20230904\Models\GetFlowResponseBody;

use AlibabaCloud\Dara\Model;
use AlibabaCloud\SDK\Appflow\V20230904\Models\GetFlowResponseBody\flow\flowNodes;
use AlibabaCloud\SDK\Appflow\V20230904\Models\GetFlowResponseBody\flow\tags;
...
class flow extends Model
{
    ...
    {
        if (\is_array($this->flowNodes)) {
            Model::validateArray($this->flowNodes);
        }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->flowNodes) {
            if (\is_array($this->flowNodes)) {
                $res['FlowNodes'] = [];
                $n1 = 0;
                foreach ($this->flowNodes as $item1) {
                    $res['FlowNodes'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['FlowNodes'])) {
            if (!empty($map['FlowNodes'])) {
                $model->flowNodes = [];
                $n1 = 0;
                foreach ($map['FlowNodes'] as $item1) {
                    $model->flowNodes[$n1] = flowNodes::fromMap($item1);
                    ++$n1;
                }
            }
        }
