...
class DescribeEnsRouteEntryListResponseBody extends Model
{
    ...
    {
        if (\is_array($this->routeEntrys)) {
            Model::validateArray($this->routeEntrys);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->routeEntrys) {
            if (\is_array($this->routeEntrys)) {
                $res['RouteEntrys'] = [];
                $n1 = 0;
                foreach ($this->routeEntrys as $item1) {
                    $res['RouteEntrys'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['RouteEntrys'])) {
            if (!empty($map['RouteEntrys'])) {
                $model->routeEntrys = [];
                $n1 = 0;
                foreach ($map['RouteEntrys'] as $item1) {
                    $model->routeEntrys[$n1] = routeEntrys::fromMap($item1);
                    ++$n1;
                }
            }
        }
