...
class DescribeStorageGatewayResponseBody extends Model
{
    ...
    {
        if (\is_array($this->storageGateways)) {
            Model::validateArray($this->storageGateways);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->storageGateways) {
            if (\is_array($this->storageGateways)) {
                $res['StorageGateways'] = [];
                $n1 = 0;
                foreach ($this->storageGateways as $item1) {
                    $res['StorageGateways'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['StorageGateways'])) {
            if (!empty($map['StorageGateways'])) {
                ...
                $n1 = 0;
                foreach ($map['StorageGateways'] as $item1) {
                    $model->storageGateways[$n1] = storageGateways::fromMap($item1);
                    ++$n1;
                }
            }
        }
