...
class DescribeExpressConnectRouterRouteEntriesResponseBody extends Model
{
    ...
    {
        if (\is_array($this->routeEntriesList)) {
            Model::validateArray($this->routeEntriesList);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->routeEntriesList) {
            if (\is_array($this->routeEntriesList)) {
                ...
                $n1                      = 0;
                foreach ($this->routeEntriesList as $item1) {
                    $res['RouteEntriesList'][$n1++] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['RouteEntriesList'])) {
            if (!empty($map['RouteEntriesList'])) {
                ...
                $n1                      = 0;
                foreach ($map['RouteEntriesList'] as $item1) {
                    $model->routeEntriesList[$n1++] = routeEntriesList::fromMap($item1);
                }
            }
        }
