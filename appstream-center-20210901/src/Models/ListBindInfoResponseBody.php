...
class ListBindInfoResponseBody extends Model
{
    ...
    {
        if (\is_array($this->bindInfoModels)) {
            Model::validateArray($this->bindInfoModels);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->bindInfoModels) {
            if (\is_array($this->bindInfoModels)) {
                $res['BindInfoModels'] = [];
                $n1 = 0;
                foreach ($this->bindInfoModels as $item1) {
                    $res['BindInfoModels'][$n1++] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['BindInfoModels'])) {
            if (!empty($map['BindInfoModels'])) {
                ...
                $n1 = 0;
                foreach ($map['BindInfoModels'] as $item1) {
                    $model->bindInfoModels[$n1++] = bindInfoModels::fromMap($item1);
                }
            }
        }
