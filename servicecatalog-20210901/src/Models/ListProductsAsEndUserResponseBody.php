...
class ListProductsAsEndUserResponseBody extends Model
{
    ...
    {
        if (\is_array($this->productSummaries)) {
            Model::validateArray($this->productSummaries);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->productSummaries) {
            if (\is_array($this->productSummaries)) {
                ...
                $n1 = 0;
                foreach ($this->productSummaries as $item1) {
                    $res['ProductSummaries'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['ProductSummaries'])) {
            if (!empty($map['ProductSummaries'])) {
                ...
                $n1 = 0;
                foreach ($map['ProductSummaries'] as $item1) {
                    $model->productSummaries[$n1] = productSummaries::fromMap($item1);
                    ++$n1;
                }
            }
        }
