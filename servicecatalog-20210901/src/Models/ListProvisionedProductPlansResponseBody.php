...
class ListProvisionedProductPlansResponseBody extends Model
{
    ...
    {
        if (\is_array($this->planDetails)) {
            Model::validateArray($this->planDetails);
        }
        parent::validate();
    }
    ...
    public function toArray($noStream = false)
    {
        ...
        if (null !== $this->planDetails) {
            if (\is_array($this->planDetails)) {
                $res['PlanDetails'] = [];
                $n1 = 0;
                foreach ($this->planDetails as $item1) {
                    $res['PlanDetails'][$n1] = null !== $item1 ? $item1->toArray($noStream) : $item1;
                    ++$n1;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['PlanDetails'])) {
            if (!empty($map['PlanDetails'])) {
                $model->planDetails = [];
                $n1 = 0;
                foreach ($map['PlanDetails'] as $item1) {
                    $model->planDetails[$n1] = planDetails::fromMap($item1);
                    ++$n1;
                }
            }
        }
